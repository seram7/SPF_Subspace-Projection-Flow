...
class BaseTrainer:
    ...
    def __init__(self, device, config):
        self.cfg = config   # put everything input cfg for flexibility
        self.device = device
        self.d_val_result = {}
    ...
    def load_model(self, model, opt):
        ...
        load_model = self.cfg.get('load_model', True)
        load_opt = self.cfg.get('load_opt', False)

        ckpt = torch.load(checkpoint)
        if 'model_state' in ckpt and load_model:
            model.load_state_dict(ckpt['model_state'], strict=False)
            print('model state loaded')
        if 'opt_state' in ckpt and load_opt:
