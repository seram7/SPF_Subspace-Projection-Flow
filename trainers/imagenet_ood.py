...
class Imagenet_OODTrainer(BaseTrainer):
    def train(self, model, opt, d_dataloaders, logger=None, logdir='', scheduler=None, clip_grad=None):
        ...
        for i_epoch in range(cfg.n_epoch):
            ...
            for x, y in indist_train_loader:
                ...
                start_ts = time.time()
                d_train = model.train_step(x, y=y, opt=opt, clip_grad=clip_grad)
                ...
                if i % cfg.val_interval == 0:
                    ...
                    if not whole_train:
                        ...
                        if cfg.save_interval is not None and i % cfg.save_interval == 0 and best_model:
                            ...
                            print(f'Iter [{i:d}] best model saved, val_loss: {val_loss} <= {best_val_loss}')
                        ...
                        if isinstance(scheduler, _LRScheduler):
                            if isinstance(scheduler, ReduceLROnPlateau):
                                ...
                                scheduler.step()
                    if whole_train:
                        train_loss = d_train['loss/train_loss_']
                        best_model = train_loss < best_train_loss
                        if cfg.save_interval is not None and i % cfg.save_interval == 0 and best_model:
                            self.save_model(model, logdir, best=best_model, i_iter=i, i_epoch=i_epoch)
                            print(f'Iter [{i:d}] best model saved, train_loss: {train_loss} <= {best_train_loss}')
